...
const drawCards_desktop = function () {
  ...
  if (allofCards == 0) {
    for (let i = 1; i <= 10; i++) {
      const innerHTML =
        '<div class="list-group-item" ><span class="leftCorner-handle"aria-hidden="true"></span>Card' +
        i +
        "</div>";
    ...
    let numofCards_mobile = $("div.inner-slider")
      .text()
      .replace(/\s/g, "")
      .split("Card");
...
const drawCards_mobile = () => {
  ...
  if (allofCards == 0) {
    ...
    let numofCards_mobile = $("div.leftside")
      .text()
      .replace(/\s/g, "")
      .split("Card");
...
sliderContainer.addEventListener("mousedown", (e) => {
  pressed = true;
  offsetX =
    e.clientX - ($("body").width() - $("div.slider-container").width()) / 2;
...
const checkBoundary = () => {
  ...
  if (
    ...
    parseInt($("div.inner-slider").width()) <
      parseInt($("div.slider-container").width())
  ) {
...
sliderContainer.addEventListener("touchstart", (e) => {
  ...
  offsetX =
    e.touches[0].clientX -
    ($("body").width() - $("div.slider-container").width()) / 2;
  startX = offsetX - innerSlider.offsetLeft;
  $("div.inner-slider").addClass("column");
  sorting();
